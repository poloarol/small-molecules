...
class CycleGAN(GAN):
    ...
    def _gan_criterion(
        self, output, target, use_lsgan: bool = True, use_wasserstein: bool = False
    ) -> float:
        assert np.sum(
        ...
        if use_wasserstein:
            ...
            loss = (tf.math.log(output + epsilon) * target) + tf.math.log(
                1 - output + epsilon
            ) * (1 - target)
    ...
    def _compute_similarity_loss(
        self, real_input, real_ouput, generated_input, generated_output
    ):
    ...
    def _loss_generator(
        ...
        (
            network_A_predict_fake,
            rec_A,
            real_A,
            network_B_predict_fake,
            rec_B,
            real_B,
            fake_A,
            fake_B,
        ...
        # Identity loss
        identity_loss: float = self._compute_similarity_loss(
    ...
    def __generator_train_function(
        self,
        discriminator_tuple,
        generator_tuple,
        real_datapoints,
        ...
        lamba_layers = [
            first_fake_predictions,
            second_generated_dp,
            real_input,
            second_fake_predictions,
            first_generated_dp,
            real_output,
            fake_input,
            fake_output,
        ...
        for layer in itertools.chain(
            first_generator_network.layers, second_generator_network.layers
        ):
            layer.trainable = True

        for layer in itertools.chain(
            first_discriminator_network.layers, second_discriminator_network.layers
        ):
            layer.trainable = False
            if isinstance(layer, layers.BatchNormalization):
                layer._per_input_updates = {}

        net_generative_partial_loss: float = lambda x: self._loss_generator(
            x, cycle_loss_weight, id_loss_weight, use_wgan=use_wgan
        )

        generator_train_function = self._train_functions(
    ...
    def __first_discriminator_train_function(
        self,
        discriminator_tuple,
        generator_tuple,
        real_datapoints,
        ...
        for layer in itertools.chain(
            first_generator_model.layers,
            second_generator_model.layers,
            second_discriminator_model.layers,
        ):
            layer.trainable = False
            if isinstance(layer, layers.BatchNormalization):
                layer._per_input_updates = {}

        net_discriminator_loss: float = lambda x: self._loss_discrimination(
            x, use_wgan=use_wgan
        )
        network_discriminator_train_function = self._train_functions(
            ...
            layer_inputs=[
                first_discriminator_prediction,
                second_discriminator_prediction,
            ],
        )

        return network_discriminator_train_function

    def __second_discriminator_train_function(
        self,
        discriminator_tuple,
        generator_tuple,
        real_datapoints,
        ...
        for layer in itertools.chain(
            first_generator_model.layers,
            second_generator_model.layers,
            first_discriminator_model.layers,
        ):
            layer.trainable = False
            if isinstance(layer, layers.BatchNormalization):
                layer._per_input_updates = {}
    ...
    def _clip_weights(self, network, clip_lambda: float = 0.1):
        weights = [
            np.clip(weight, -clip_lambda, clip_lambda)
            for weight in network.get_weights()
        ]
        network.set_weights(weights)

    def train_step(self, inputs, batches_tuple, generator_tuple, discriminator_tuple):
        ...
        second_discriminator_train_function = (
            self.__second_discriminator_train_function()
        )
