...
class GCN(torch.nn.Module):
    ...
    def __init__(
        self, n_features: int, hidden_channels: int, dropout: float = 0.2
    ) -> None:
...
class GAT(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv_one = GATConv(
            n_features, hidden_channels, heads=heads, dropout=dropouts
        )
        self.conv_two = GATConv(
            hidden_channels * heads,
            int(hidden_channels / 2),
            heads=heads,
            dropout=dropouts,
        )
        self.conv_three = GATConv(
            int(hidden_channels / 2) * heads,
...
class Trainer:
    ...
    def run(self, n_epochs: int = 10):
        ...
        for epoch in range(1, n_epochs + 1):
            epoch_loss, epoch_auc, progress_tracker = self.train_one_epoch(epoch=epoch)
