...
from converter import GraphConverter, Descriptors
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, data, mapping, max_length: int, batch_size: int = 32, shuffle: bool = True) -> None:
        super().__init__()
        self.index: int
        self.data = data
    ...
    def data_generation(self, batch) -> Tuple:
        ...
        x_1 = np.empty((self.batch_size, Descriptors.BOND_DIM, self.max_length, self.max_length))
        x_2 = np.empty((self.batch_size, self.max_length, len(self.mapping)))
        x_3 = np.empty((self.batch_size, ))

        for i, batch_id in enumerate(batch):
            x_1[i, ], x_2[i, ], x_3[i, ] = self.load(batch_id)
            return [x_1, x_2], x_3
